...
function Artiste() {
            ...
            {enrengistrements.map(item => (
                ...
                <div 
                 key={item.id}>
                    ...
                    <div className="photo" onClick={() => imageClicked(item)}><img src={item.photo} alt='photo'/></div>
                </div>
            ))}
    ...
    function imageClicked(image){
       ...
       fetch(`http://localhost:8000/artists/${image.id}`).then(reponse => {
        return reponse.json()
       }).then(v => {
            window.location.href = `../html/détailArtiste.html?id=${v.id}`
        })
...
ReactDOM.render(< Artiste />, document.querySelector('.centre'));